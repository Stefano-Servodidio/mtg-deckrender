...
import { getUniqueCards, sleep, prepareCardOperations, prepareCountOperations } from '../utils/api'
...
describe('API Utilities', () => {
    ...
    describe('prepareCardOperations', () => {
        it('should calculate correct positions for main deck cards', () => {
            const mockImages = [
                { buffer: Buffer.from('test1'), quantity: 1, type: 'main' as const },
                { buffer: Buffer.from('test2'), quantity: 1, type: 'main' as const },
                { buffer: Buffer.from('test3'), quantity: 1, type: 'main' as const }
            ]
        ...
        it('should calculate correct positions for sideboard cards', () => {
            const mockImages = [
                { buffer: Buffer.from('test1'), quantity: 1, type: 'sideboard' as const }
            ]
    ...
    describe('prepareCountOperations', () => {
        it('should generate operations only for cards with quantity > 1', () => {
            const mockImages = [
                { buffer: Buffer.from('test1'), quantity: 1, type: 'main' as const },
                { buffer: Buffer.from('test2'), quantity: 4, type: 'main' as const },
                { buffer: Buffer.from('test3'), quantity: 2, type: 'main' as const }
            ]
        ...
        it('should use fallback count icon for unknown quantities', () => {
            const mockImages = [
                { buffer: Buffer.from('test1'), quantity: 7, type: 'main' as const }
            ]
        ...
        it('should handle sideboard cards with correct positioning', () => {
            const mockImages = [
                { buffer: Buffer.from('test1'), quantity: 3, type: 'sideboard' as const }
            ]
