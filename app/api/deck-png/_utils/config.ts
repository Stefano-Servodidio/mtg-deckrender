...
export function calculateCanvasDimensions(
    ...
    const { card, spacing, row } = DECK_LAYOUT_CONFIG
    
    const canvasWidth = 
        card.width * cardsPerRow + 
        spacing.betweenCards * (cardsPerRow - 1) + 
        spacing.canvasPadding * 2
    
    const rowHeight = card.height * row.heightMultiplier
    
    const canvasHeight = 
        rowHeight * totalRows +
        spacing.betweenCards * (totalRows - 1) +
        spacing.canvasPadding * 2 +
        (hasSideboard ? spacing.sideboardSeparator + 2 * rowHeight : 0)
    
    return { width: canvasWidth, height: canvasHeight }
}

export function calculateRowHeight(): number {
    return DECK_LAYOUT_CONFIG.card.height * DECK_LAYOUT_CONFIG.row.heightMultiplier
}