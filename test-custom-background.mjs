...
async function testCustomBackgroundImage() {
    try {
        ...
        console.log(
            '✅ Created background canvas from image (resized to 1080x1080)'
        )
        ...
        const finalImage = await backgroundCanvas
            .composite([
                {
        ...
        console.log(
            `✅ Final image dimensions: ${metadata.width}x${metadata.height}`
        )

        // Save for visual inspection
        await sharp(finalImage).toFile('/tmp/test-custom-background-result.png')
        console.log(
            '✅ Saved test result to /tmp/test-custom-background-result.png'
        )
